...
# 检查数据文件
if [ ! -f "data/all-content.json" ]; then
    echo "⚠️  警告：找不到 data/all-content.json"
    echo "AI 查询功能将无法正常工作"
fi
...
# 创建 package.json (用于 Cloudflare Pages Functions)
...
# 创建 _redirects
echo "/api/* /api/:splat 200" > "$TEMP_DIR/public/_redirects"
echo "/* /index-vercel.html 200" >> "$TEMP_DIR/public/_redirects"
...
for i in {1..30}; do
    ...
    if [ "$DEPLOYMENT_STATUS" = "success" ]; then
        ...
        echo "4. 添加：ZHIPU_API_KEY = a3b94c17d28e4940b5102294f1d9cf61.uB51O7OoEEejjWlL"
        echo ""
        break
